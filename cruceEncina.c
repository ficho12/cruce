...
int main (int argc, char *argv[]){
	...
	if(!verify(argv[2]))
	{
		...
		printf("Error. El parametro velocidad debe ser un número positivo.\n\n");
		ayuda();
		return 2;
	}
	...
	if(!verify(argv[1]))
	{
		...
		printf("Error. El parametro de procesos debe ser un número positivo.\n\n");
		ayuda();
		return 3;
	}
	...
	if(semctl(datos.semid,1,SETVAL,arg) == -1)
	{
	...
	for(i=2;i<6;i++){
		if(semctl(datos.semid,i,SETVAL,arg) == -1)
		{
